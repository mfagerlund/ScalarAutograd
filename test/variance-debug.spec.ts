...
describe('Variance Debug Test', () => {
  it('should debug energy values', () => {
    ...
    console.log(`Sphere: ${sphere.vertices.length} vertices`);
    ...
    for (const faceIdx of star) {
      ...
      console.log(`  Face ${faceIdx}: N=(${normal.x.data.toFixed(3)}, ${normal.y.data.toFixed(3)}, ${normal.z.data.toFixed(3)}), θ=${angle.data.toFixed(3)}`);
    ...
    console.log(`\nCovariance matrix:`);
    console.log(`  [${c00.data.toFixed(4)}, ${c01.data.toFixed(4)}, ${c02.data.toFixed(4)}]`);
    console.log(`  [${c01.data.toFixed(4)}, ${c11.data.toFixed(4)}, ${c12.data.toFixed(4)}]`);
    console.log(`  [${c02.data.toFixed(4)}, ${c12.data.toFixed(4)}, ${c22.data.toFixed(4)}]`);
    ...
    console.log(`Frobenius norm: ${sqrtFrob.data.toFixed(4)}`);

    const lambdaMinProxy = V.sub(trace, sqrtFrob);
    console.log(`λ_min proxy (trace - ||C||_F): ${lambdaMinProxy.data.toFixed(4)}`);

    const energy = V.square(lambdaMinProxy);
    console.log(`Energy (squared): ${energy.data.toExponential(4)}`);

    const actualEnergy = DevelopableEnergy.computeVertexEnergy(vertexIdx, sphere);
    console.log(`Actual energy from function: ${actualEnergy.data.toExponential(4)}`);
  });
});
